...
@section('title', __('web.offline.title'))
...
<div class="grid min-h-dvh place-items-center px-6">
    <div class="glass card max-w-sm text-center">
        ...
        <h1 class="mt-5 text-lg font-bold">{{ __('web.offline.heading') }}</h1>
        <p class="mt-3 text-sm leading-7 text-ink-400">{{ __('web.offline.body') }}</p>
        <button type="button" class="btn btn-primary mt-6 w-full" onclick="location.reload()">{{ __('web.offline.retry') }}</button>
    </div>
</div>
@endsection
