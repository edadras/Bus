    ...
    <div class="grid gap-3 lg:grid-cols-[1fr_320px]">
        ...
        <div class="glass card flex flex-col">
            <div class="flex items-center justify-between">
                <h2 class="text-sm font-bold">ناوگان در حال حرکت</h2>
            ...
            <input type="search" class="field mt-3 !py-2 text-xs" placeholder="جستجوی شماره اتوبوس یا خط"
                   x-model="busFilter">
                ...
                <template x-for="bus in filteredBuses" :key="bus.trip_uuid">
                    <button type="button" class="rounded-xl bg-white/[0.03] px-3 py-2.5 text-start transition hover:bg-white/[0.06]"
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-sm font-semibold" x-text="`اتوبوس ${$num(bus.bus_number)}`"></span>
                            <span class="ms-auto text-[10px] text-ink-500" x-text="`خط ${$num(bus.line_code)}`"></span>
                        </div>
                        <p class="mt-1 truncate text-[11px] text-ink-400" x-text="bus.driver_name || 'راننده نامشخص'"></p>
                        <div class="mt-1.5 flex items-center gap-3 text-[10px] text-ink-500">
                            <span x-text="`${$num(bus.passenger_count)} مسافر`"></span>
                            <span x-text="bus.speed ? `${$num(Math.round(bus.speed))} km/h` : '—'"></span>
                            <span x-show="bus.is_off_route" class="text-amber-400">خارج از مسیر</span>
                            <span x-show="bus.is_idle" class="text-amber-400">متوقف</span>
                        </div>
                    </button>
                </template>
                <p x-show="!filteredBuses.length" class="py-8 text-center text-sm text-ink-500">
                    هیچ اتوبوسی در حال سرویس نیست.
                </p>
            </div>
        </div>
    </div>
